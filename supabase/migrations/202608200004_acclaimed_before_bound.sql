-- Take known stories before the pool fills.
--
-- Exempting acclaimed issues from the per-volume cap left them exposed to the
-- overall 900-row limit, which still cut Hush at five of its twelve issues. The
-- ordering now takes every acclaimed issue first and round-robins the remainder,
-- so a story we have identified is never delivered in part.
...
language sql stable as $$
  with requested as (
    ...
    select
      ...
      i.volume_id,
      w.co_issue_count,
      exists (select 1 from issue_acclaim a where a.issue_id = i.id) as is_acclaimed,
      row_number() over (
    ...
    -- A known story is exempt from the per-volume cap. Hush is twelve issues;
    -- truncating it at the cap produced a candidate titled #608-612, which is
    -- not Hush and misrepresents it as a five-issue read.
    where position_in_volume <= 40 or is_acclaimed
    -- Known stories first, then round robin: every volume's opening issue, then
    -- every volume's second, and so on. Exempting acclaimed issues from the
    -- per-volume cap was not enough on its own, because the overall limit still
    -- cut Hush off mid-story; there are few enough of them that taking them all
    -- first costs the rest almost nothing.
    order by (not is_acclaimed), position_in_volume, co_issue_count desc, volume_id
    limit 900
  )
  select
    i.id,
...
comment on function reading_path_issue_candidates(uuid[]) is
  'At most 900 issues containing every requested character: acclaimed issues first, then round robin across volumes.';
