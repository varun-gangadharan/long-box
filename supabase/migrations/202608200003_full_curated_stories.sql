-- Never truncate a known story.
--
-- The per-volume cap kept one ongoing title from filling the pool, but it also
-- cut Hush off at five of its twelve issues, producing a candidate labelled
-- "Batman #608-612" that claims to be a five-issue read. A story we have
-- explicitly identified is exactly what the pool is for, so it is exempt.
...
comment on function reading_path_issue_candidates(uuid[]) is
  'At most 900 issues containing every requested character, round robin across volumes, with acclaimed issues first and never truncated.';
